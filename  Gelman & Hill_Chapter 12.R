...
radon <- read.csv("cty.dat copy.csv")
colnames(radon)
...
srrs2 <- read.table ("srrs2.dat.csv", header=T, sep=",")
mn <- srrs2$state=="MN"
...
cty <- read.table ("cty.dat.csv", header=T, sep=",")
...
cty.sds = mean(sqrt(cty.vars[!is.na(cty.vars)]))/sqrt(sample.size) # not able to run this because have not specified sample size
